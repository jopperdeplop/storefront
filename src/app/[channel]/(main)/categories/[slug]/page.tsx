...
// --- FIX: Strict Types for Content Blocks ---
...
export default async function Page(props: { params: Promise<{ slug: string; channel: string }> }) {
	...
	// We initialize as an empty array so we never map over null/any.
	let descriptionBlocks: EditorJsBlock[] = [];

	if (description) {
		try {
			// We cast to unknown first, then check shape, or assert if we trust the source.
			// Here we treat it as a partial object with a blocks array.
			const parsed = JSON.parse(description) as { blocks: EditorJsBlock[] };
						...
						{descriptionBlocks.length > 0 && (
							<div className="mt-16 border-t border-gray-200 pt-12">
								...
								<div className="prose prose-sm prose-neutral max-w-none prose-headings:font-serif prose-a:text-terracotta">
									{/* FIX: Explicitly type 'block' to allow safe access to .id and .data */}
									{descriptionBlocks.map((block: EditorJsBlock) => (
